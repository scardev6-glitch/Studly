...
async function getOverallStats(req, res) {
  try {
    ...
    const averageMastery = totalTopics > 0 
      ? Math.round(progressList.reduce((acc, curr) => acc + curr.masteryLevel, 0) / totalTopics)
      : 0;
    
    const totalQuizzes = progressList.reduce((acc, curr) => acc + curr.totalAttempts, 0);
...
async function getTopicProgress(req, res) {
  try {
    const userId = req.user.id;
    const progressList = await UserProgress.find({ userId }).populate('topicId');
...
async function getSubjectProgress(req, res) {
  try {
    ...
    progressList.forEach(p => { progressByTopic[p.topicId.toString()] = p; });

    const subjects = {};
    topics.forEach(topic => {
      if (!subjects[topic.subject]) {
        subjects[topic.subject] = { subject: topic.subject, totalTopics: 0, completedTopics: 0, masterySum: 0, topics: [] };
      }
      ...
      subjects[topic.subject].topics.push({
        ...
        isCompleted,
        isStarted: !!prog,
        lastQuizScore: prog?.correctAttempts > 0 ? Math.round((prog.correctAttempts / (prog.correctAttempts + prog.mistakes)) * 100) : 0,
    ...
    const result = Object.values(subjects).map(s => ({
      ...s,
      averageMastery: s.totalTopics > 0 ? Math.round(s.masterySum / s.totalTopics) : 0,
      completionPercentage: s.totalTopics > 0 ? Math.round((s.completedTopics / s.totalTopics) * 100) : 0,
...
async function getGameStats(req, res) {
  try {
    ...
    if (!user) return res.json({ level: 1, totalXp: 0, aiCredits: 5, points: 0, nextLevelXp: 100, currentLevelXp: 0 });
    ...
    res.json({
      ...
      nextLevelXp,
      currentLevelXp,
      xpProgress: Math.min(100, Math.round((currentLevelXp / (nextLevelXp - prevLevelXp)) * 100))
    });
  } catch (error) {
    console.error('getGameStats Error:', error.message);
    res.json({ level: 1, totalXp: 0, aiCredits: 5, points: 0, nextLevelXp: 100, currentLevelXp: 0, xpProgress: 0 });
  }
}
