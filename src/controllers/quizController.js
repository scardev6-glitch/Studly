...
async function getQuiz(req, res) {
  try {
    ...
    const questions = await quizEngine.generateQuiz(req.user.id, session.topicId);
    res.json(questions);
  } catch (error) {
...
async function submitQuiz(req, res) {
  try {
    ...
    const questions = await quizEngine.generateQuiz(userId, session.topicId._id);
    ...
    for (const attempt of results.analysis) {
      if (!attempt.isCorrect) {
        const questionObj = questions.find(q => q._id.toString() === attempt.questionId.toString());
        attempt.aiExplanation = await aiEngine.generateExplanation(questionObj, attempt.userAnswer, attempt.correctAnswer);
      } else {
        // Award XP + AI credits for correct answers
        await gamificationEngine.awardXp(userId, 10, `Correct answer in ${session.topicId.name}`);
      }
    }

    const progress = await progressEngine.updateMastery(userId, session.topicId._id, results);
